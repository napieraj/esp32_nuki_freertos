...
lock::LockState NukiProLock::nuki_to_esphome_state(NukiLock::LockState s) {
    switch (s) {
        case NukiLock::LockState::Locked:      return lock::LOCK_STATE_LOCKED;
        case NukiLock::LockState::Unlocked:
        case NukiLock::LockState::Unlatched:   return lock::LOCK_STATE_UNLOCKED;
        case NukiLock::LockState::MotorBlocked: return lock::LOCK_STATE_JAMMED;
        case NukiLock::LockState::Locking:     return lock::LOCK_STATE_LOCKING;
        case NukiLock::LockState::Unlocking:
        case NukiLock::LockState::Unlatching:  return lock::LOCK_STATE_UNLOCKING;
        default:                               return lock::LOCK_STATE_NONE;
    }
}
...
void NukiProLock::setup() {
    ...
    if (this->paired_.load()) {
        ...
        ESP_LOGI(TAG, "Not paired — enable pairing mode on the lock");
    ...
    ESP_LOGI(TAG, "Ready — BLE task on Core 0");
}
...
void NukiProLock::control(const lock::LockCall &call) {
    ...
    if (call.get_state().has_value()) {
        ...
        if (s == lock::LOCK_STATE_LOCKED)   cmd.type = NukiCommandType::LOCK;
        if (s == lock::LOCK_STATE_UNLOCKED) cmd.type = NukiCommandType::UNLOCK;
    }
...
// ── notify() — BLE context (Core 0), only touches atomics ─────────────
...
void NukiProLock::nuki_task() {
    ...
    while (true) {
        ...
        if (xQueueReceive(this->command_queue_, &cmd, poll_ticks) == pdTRUE) {
            ...
            // Timeout path — aggressive polling
            ...
            if (this->paired_.load() && this->status_poll_requested_.exchange(false)) {
                this->do_status_poll();
            }
        }
    }
...
void NukiProLock::do_status_poll() {
    ...
    if (r == Nuki::CmdResult::Success) {
        ...
        ESP_LOGD(TAG, "State: %d  Bat: %d%%  RSSI: %d",
                 (int)this->key_turner_state_.lockState,
                 this->nuki_lock_.getBatteryPerc(), this->nuki_lock_.getRssi());
    } else {
